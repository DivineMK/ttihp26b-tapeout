...
// TinyQV CPU and memory controller wrapper
// - passes strap_mode down into mem_ctrl for backend select
module tinyQV (
    input clk,
    input rstn,
    ...
    // Strap select from top level, sampled at reset, stable in run
    // - 0 for single-SPI emulator path, 1 for QSPI PMOD path
    input         strap_mode,
