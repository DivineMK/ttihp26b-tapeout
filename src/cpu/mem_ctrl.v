/* TinyQV: A RISC-V core designed to use minimal area.
   ...
   Dual-backend (our change): the original design only had the QSPI controller talking
   to the PMOD. We add a plain single-SPI engine (spi_mem_ctrl) for the RP2040 emulator using spi-ram-emu:
   - strap_mode for select
   - expose the same byte-level handshake to CPU
 */
...
module tinyqv_mem_ctrl (
    ...
    // Board-level choice, sampled once at reset exit in project.v and held steady
    // afterwards. Never toggles mid-run.
    input            strap_mode, // 0 = Single-SPI (spi-ram-emu), 1 = QSPI (PMOD)
    ...
    // QSPI Controller (PMOD)
    wire [3:0] q_spi_data_out;
    wire [3:0] q_spi_data_oe;
    wire       q_spi_clk_out;
    wire       q_spi_flash_select;
    wire       q_spi_ram_a_select;
    wire       q_spi_ram_b_select;
    wire [7:0] q_data_out;
    wire       q_data_req;
    wire       q_data_ready;
    wire       q_busy;
    ...
    qspi_controller q_ctrl(
        ...
        .spi_data_out(q_spi_data_out),
        .spi_data_oe(q_spi_data_oe),
        .spi_clk_out(q_spi_clk_out),

        .spi_flash_select(q_spi_flash_select),
        .spi_ram_a_select(q_spi_ram_a_select),
        .spi_ram_b_select(q_spi_ram_b_select),

        .addr_in(addr_in),
        .data_in(data_to_write[{write_qspi_data_byte_idx,3'b000} +:8]),
        .start_read(strap_mode ? (start_read || start_instr) : 1'b0),
        .start_write(strap_mode ? start_write : 1'b0),
        .stall_txn(strap_mode ? (stall_txn || data_stall) : 1'b0),
        .stop_txn(strap_mode ? stop_txn : 1'b0),

        .data_out(q_data_out),
        .data_req(q_data_req),
        .data_ready(q_data_ready),
        .busy(q_busy)
    );

    // Single-SPI engine for RP2040 emulator using spi-ram-emu
    // - same byte handshake as QSPI block, 0x03 read / 0x02 write, 16-bit addr
    // - enabled when strap_mode is 0
    // Single-SPI Controller (23LC512 / spi-ram-emu)
    wire [3:0] s_spi_data_out;
    wire [3:0] s_spi_data_oe;
    wire       s_spi_clk_out;
    wire       s_spi_cs;
    wire [7:0] s_data_out;
    wire       s_data_req;
    ...
    spi_mem_ctrl #(
        ...
        .spi_data_oe(s_spi_data_oe),
        .spi_clk_out(s_spi_clk_out),
        .spi_cs(s_spi_cs),
    ...
    assign qspi_data_out   = strap_mode ? q_data_out   : s_data_out;
    assign qspi_data_req   = strap_mode ? q_data_req   : s_data_req;
    assign qspi_data_ready = strap_mode ? q_data_ready : s_data_ready;
    assign qspi_busy       = strap_mode ? q_busy       : s_busy;

    assign spi_clk_out      = strap_mode ? q_spi_clk_out      : s_spi_clk_out;
    assign spi_flash_select = strap_mode ? q_spi_flash_select : s_spi_cs;
    assign spi_ram_a_select = strap_mode ? q_spi_ram_a_select : 1'b1; // active-low -> 1 = not selected
    assign spi_ram_b_select = strap_mode ? q_spi_ram_b_select : 1'b1; // active-low -> 1 = not selected
    assign spi_data_out     = strap_mode ? q_spi_data_out     : s_spi_data_out;
    assign spi_data_oe      = strap_mode ? q_spi_data_oe      : s_spi_data_oe;
